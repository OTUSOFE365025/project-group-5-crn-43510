...
| **Primary Functional Requirements** | From the use cases presented, the primary drivers were determined to be:  * **UC-1 (Access Academic Details):** Because it directly supports the core business of providing personalized, accurate student information via a natural-language interface.  * **UC-2 (Receive Notifications):** Because it is critical for engaging users and ensuring timely communication, directly impacting the platform's utility.  * **UC-5 (Regulate System Integrations):** Because it is crucial for system operations, ensuring secure and reliable connectivity with external institutional systems (LMS, Registration) and maintaining data integrity. |
| **Quality Attribute Scenarios** | The scenarios were described in the Quality Attribute Scenarios section. They have now been prioritized (based on the provided Importance and Difficulty ratings) as follows:  * **QA-1 (Performance)**: Importance: **High**, Difficulty: **High** * **QA-2 (Availability)**: Importance: **High**, Difficulty: **High** * **QA-3 (Security)**: Importance: **High**, Difficulty: **High** * **QA-4 (Scalability)**: Importance: **High**, Difficulty: **Medium** * **QA-5 (Modifiability)**: Importance: **Medium**, Difficulty: **Medium** |
| **Constraints** | All of the operational and technical boundaries discussed in the System Constraints section are included as drivers:  * **CON-1 (Scalability Requirement):** Support $\ge 5,000$ concurrent users.  * **CON-2 (Cross-Platform Access):** Seamless operation across web and mobile browsers.  * **CON-3 (Cloud Deployment Environment):** Hosted on a university-approved cloud service.  * **CON-4 (Privacy and Compliance):** Strict adherence to institutional policies.  * **CON-5 (Performance Target):** Response time $\le 2$ seconds for most queries.  * **CON-6 (Unified AI Backend Models):** Reliance on a single underlying AI backend for both voice and text. |
| **Architectural Concerns** | All of the identified design priorities and risks from the Architectural Concerns section are included as drivers:  * **CRN-1:** Define Overall Microservice and Integration Structure.  * **CRN-2:** Utilize Team’s Knowledge of Python (**FastAPI**) and **React** for UI.  * **CRN-3:** Ensure Secure Handling of **SSO** and Personally Identifiable Information.  * **CRN-4:** Design for Scalability and Cloud Auto-Deployment.  * **CRN-5:** Simplify Integration of Future AI Models and External APIs.  * **CRN-6:** Provide Clear Module Responsibility Allocation for Team Members. |
