...
| **CRN-2** | **Utilize Team’s Knowledge of Python (FastAPI) and React for UI** | Utilize technologies already familiar to the development team (FastAPI for backend microservices and React for the web interface) to accelerate implementation and reduce learning overhead. This also ensures maintainable code and rapid prototyping aligned with existing expertise. |
| **CRN-3** | **Ensure Secure Handling of SSO and Personally Identifiable Information** | Design the authentication and data-handling components to comply with institutional security standards and privacy laws. All SSO tokens and user data must be encrypted in transit and at rest, and access must follow role-based authorization. |
